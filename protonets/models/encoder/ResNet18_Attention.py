...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, training=False):
        super(BasicBlock, self).__init__()
        self.training = training
        self.stride = stride
        # self.conv1 = nn.Conv2d(in_channels, out_channels, padding='same', kernel_size=3, stride=2)
        self.conv1 = Conv2dSame(in_channels=in_channels, out_channels=out_channels, kernel_size=(7, 7), stride=(stride, stride), groups=1, bias=True)
        self.bn1 = nn.BatchNorm2d(out_channels, affine=self.training)

        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding='same', stride=1)
        ...
        if stride != 1 or in_channels != self.expansion * out_channels:
            ...
            # self.shortcut = loads(dumps(self.shortcut))
...
class ResNet(nn.Module):
    ...
    def build_resblock(self, blocks, out_channels, num_blocks, stride):
        ...
        for stride in strides:
            res.append(blocks(self.in_channels, out_channels, stride))
