...
#update size kernel: change kernel size (3,3) -> (3,1), (7,7) -> (7,1)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, training=False):
        ...
        self.conv1 = Conv2dSame(in_channels=in_channels, out_channels=out_channels, kernel_size=(7, 1), stride=(stride, stride), groups=1, bias=True)
        self.bn1 = nn.BatchNorm2d(out_channels, affine=self.training)

        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=(3,1), padding='same', stride=1)
