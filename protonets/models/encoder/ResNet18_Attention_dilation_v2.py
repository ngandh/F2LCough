...
class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        self.conv1 = nn.Conv2d(2, 1, kernel_size=kernel_size, padding='same', stride=1) # activation=tf.nn.sigmoid
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, dilation=1, training=False):
        ...
        self.conv1 = Conv2dSame(in_channels=in_channels, out_channels=out_channels, kernel_size=(7, 7), stride=(stride, stride), groups=1, bias=True, dilation=dilation)
        self.bn1 = nn.BatchNorm2d(out_channels, affine=self.training)

        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding='same', stride=1, dilation=dilation)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, strides=1):
        ...
        self.conv2 = nn.Conv2d(out_channels, 3, strides, padding='same')
...
class ResNet(nn.Module):
    ...
    def __init__(self, blocks, layer_dims, num_classes=9, training = False):
        ...
        self.stem = nn.Sequential(nn.Conv2d(1, 64, kernel_size=3, padding='same', stride=1),
        ...
        self.layer2 = self.build_resblock(blocks, 128, layer_dims[1], stride=2, dilation = 1)
        self.layer3 = self.build_resblock(blocks, 256, layer_dims[2], stride=2, dilation = 2)
        self.layer4 = self.build_resblock(blocks, 512, layer_dims[3], stride=2, dilation = 4)
