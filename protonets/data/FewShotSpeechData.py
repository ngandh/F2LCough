...
class FewShotSpeechDataset(TransformDataset):
    ...
    def __init__(self, data_dir, class_file, n_support, n_query, cuda, args):
        ...
        # self.gfcc = self.build_gfcc_extractor()
        self.lfcc = self.build_lfcc_extractor()
    ...
    #     noise_path = "/content/drive/MyDrive/cough_detection/few-shot-fed/few-shot-ho/data/speech_commands"
    #     background_path = os.path.join(noise_path, '..' , '_background_noise_', '*.wav')
    #     background_path = "/content/drive/MyDrive/cough_detection/few-shot-fed/few-shot-ho/data/speech_commands/_background_noise_/*.wav"
    #     background_data = []
    ...
    def load_background_data(self):
        ...
        background_path =  os.path.join("/content/drive/MyDrive/cough_detection/few-shot-fed/few-shot-ho/data/speech_commands/_background_noise_", '*.wav')
        background_data = []
        if self.use_background or self.silence:
            for wav_path in glob.glob(background_path):
                bg_sound, bg_sr = torchaudio.load(wav_path)
    ...
    def build_lfcc_extractor(self):
        # frame_len = self.window_size_ms / 1000
        # stride = self.window_stride_ms / 1000
        lfcc =torchaudio.transforms.LFCC(sample_rate = 16000, n_filter = 128, 
                      f_min = 0.0, 
                      n_lfcc = 40, dct_type = 2, 
                      norm = 'ortho', log_lf = False
                      )
        
        return lfcc

    def build_gfcc_extractor(self, sig = None):
        # init input vars
        num_ceps = 13
        low_freq = 0
        high_freq = 2000
    ...
    def mix_background(self, use_background, k, d):
        foreground = d[k]
        if use_background:
            # print("lendatane ", len(self.background_data))
    ...
    def extract_features(self, k, d):
        ...
        # features = self.gfcc(d[k])
        # features = self.lfcc(d[k])[0]
        # print("mfcc: ", features.shape)
        if self.triplet:
          emb_space = self.load_triplet_model()
          with torch.no_grad():
            x = features
            spec = features.reshape(1, x.shape[0], x.shape[1])
        ...
        if d['class'] not in self.data_cache:
            if d['class'] == '_silence_':
                ...
                if d['class'] == '_unknown_':
                    ...
                    unknown_dir = "/content/drive/MyDrive/cough_detection/few-shot-fed/few-shot-ho/data/speech_commands/core/_unknown_"
